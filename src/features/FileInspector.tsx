  ...
  return (
    <>
      <aside className="file-inspector">
        ...
        <div className="inspector-actions"><Button tone="primary" onClick={read} disabled={busy} trailing={<Eye size={15} weight="light" />}>Preview</Button><button className="icon-button" onClick={download} aria-label="Download"><DownloadSimple size={20} weight="light" /></button><button className="icon-button" onClick={onShare} aria-label="Share"><ShareNetwork size={20} weight="light" /></button></div>
        ...
        <section className="inspector-section"><header><h3><UsersThree size={18} weight="light" /> Access ({grants.filter((grant) => !grant.revokedAt).length})</h3><button onClick={onShare}>Manage</button></header>{grants.length === 0 ? <p className="empty-copy">Only you can access this file.</p> : grants.slice(0,3).map((grant) => <div className="grant-row" key={grant.id}><span>{grant.recipientLabel.slice(0, 1).toUpperCase()}</span><div><strong>{grant.recipientLabel}</strong><small>{grant.revokedAt ? 'Revoked' : grant.permissions.join(' · ')}</small></div>{!grant.revokedAt && <button onClick={() => actions.revokeGrant(grant.id)}>Revoke</button>}</div>)}</section>
        <section className="inspector-section"><header><h3><ClockCounterClockwise size={18} weight="light" /> Versions ({versions.length})</h3><button onClick={() => versionInput.current?.click()}>Add</button></header><input className="sr-only" type="file" ref={versionInput} onChange={(event) => { const selected = event.target.files?.[0]; if (selected) actions.addVersion(file.id, selected); }} />{versions.slice(0,4).map((version) => <div className="version-row" key={version.id}><strong>v{version.version}</strong><span>{formatDate(version.createdAt, true)}</span>{version.id === file.currentVersionId && <em>Current</em>}</div>)}</section>
